...
// custom types
const status: Array<string> = [ 'all','confirmed','unconfirmed','partial' ]
@Component({
  components: {
...
export class TransactionListTs extends Vue {
   ...
   * @return {void}
   */
  public async getTransactionListByOption(option) {
    this.selectedOption = status.includes(option) ? option : 'all'
    if (status.includes(option)) {
      const action = 'wallet/GET_ALL_TRANSACTIONS'
      await this.$store.dispatch(action, {
        group: this.selectedOption,
